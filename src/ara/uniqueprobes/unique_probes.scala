...
object uniqueProbes {
  ...
  def main(args: Array[String]) {
    ...
    val uniqueProbes = kmerTools.orthologGenomeUniqueKmers(orthologous_kmers, genomeKmerCounts)

      // Find nonredundant markers.
      // If requested, skip this step
    val outProbes = if (redundant) { uniqueProbes } else { kmerTools.nonRedundantKmers(uniqueProbes, k) }

      // Output
    output(outProbes, inputFastas.length, out_prefix)
  ...
  def output(probes: List[(List[String], List[Set[kmerType]])], n_genomes: Int, prefix: String): Unit = {
    ...
    for(genome_i <- 0 until n_genomes) {
     ...
     println("--------Unique Markers for genome " + genome_i.toString + "--------")
      ...
      for(gene_i <- 0 until probes.length){
        ...
        for(probe_i <- gene_probes.sortWith(_.index < _.index)) {
          println(probes(gene_i)._1(genome_i) + '\t' + probe_i.index.toString + '\t' + probe_i.seq)
          outfd.write(probes(gene_i)._1(genome_i) + '\t' + probe_i.index.toString + '\t' + probe_i.seq + '\n')
          outfd_fasta.write(">" + probes(gene_i)._1(genome_i) + '_' + probe_i.index.toString  + '\n');
          outfd_fasta_all.write(">" + probes(gene_i)._1(genome_i) + '_' + probe_i.index.toString  + '\n');
  ...
  def usage(arg0: String): Unit = {
    ...
    println("Usage: " + arg0 + " <sequence1>[,sequence2[,...]] <genome1>[,genome2[,...]] <k> <out_prefix> [R]");
